...
def tumoneri_fasttext(onerilen_articles):
    import torch
    ...
    # Ortalama vektöR İLGİLENDİĞİM MAKALELERİN hesapla
    ...
    def generate_combined_vector(words, model, target_dim=300):
        ...
        """
    # Kelimelerin vektörlerini hesapla
        embeddings = []
        for word in words:
        ...
        if len(embeddings) == 0:
            return np.zeros(target_dim)  # Hiçbir kelime bulunamadıysa sıfırlar
    ...
    #######################################################!!!!!!!ONERİLEN ARTİCLES
    # Önerilen makalelerin vektörlerini hesapla
    onerilen_makale_vetorleri = []
    for article in onerilen_articles:
        onerilen_makale_embeddings = generate_embeddings(preprocess_text(article), modelfasttext)
        mean_embedding = np.mean(onerilen_makale_embeddings, axis=0)
        onerilen_makale_vetorleri.append(mean_embedding)

    # Ortalama vektörü hesapla
    ortalama_onerilen_makale_vektor = np.mean(onerilen_makale_vetorleri, axis=0)
    ...
    ortalama_ilgiarticles_vektor_tensor = torch.tensor((ortalama_ilgiarticles_vektor+ ortalama_onerilen_makale_vektor)/2)
    ...
    oneri = {
        "id": giren_sifresi,
        "fasttext_tum_oneri": fasttext_baslangic_oneri()
    }

    # Veritabanında "oneri" adlı anahtar altında oluşturulan yapıyı gönder
    database.child("tum_fasttextonerisi").child(giren_sifresi).set(oneri)    